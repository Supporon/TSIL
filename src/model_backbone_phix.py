...
# Domain-knowledge temporal predicates (momentum, volatility, return-distribution,
# trend, ...). Optional: if the module is present, its predicates become available
# via the `phi_type` config key. The registry below is the set of keys it handles.
try:
    from src.custom_phi_constructors import get_custom_I_P
    _CUSTOM_PHI_TYPES = {
        'phi_momentum', 'phi_volatility', 'phi_return_dist', 'phi_trend',
        'phi_multiscale', 'phi_temporal', 'phi_feature_corr', 'phi_price_volume',
        'phi_factor', 'phi_technical', 'phi_vol_return', 'phi_return_sim',
        'phi_market_beta', 'phi_cross_corr', 'phi_lead_lag', 'phi_common_factor',
        'phi_vol_sync', 'phi_tail_risk', 'phi_market_regime', 'phi_sector_cluster',
    }
...
class WeightedMSELoss(nn.Module):
    ...
    def forward(self, phi_type, batch_x, timestamp_x, outputs, targets, tau_hat=None, tau=None):
        ...
        if 'phi_x' in phi_type:
            ...
            # Domain-knowledge predicates (momentum / volatility / return-distribution /
            # trend / multiscale / temporal / ... ) implemented in
            # src/custom_phi_constructors.py. These are the paper-aligned temporal
            # predicates; dispatch on the exact phi_type key registered there.
