...
def main(seed, config, config_file, ind):
# def main(seed, config_file="configs/config_patchtst.yaml"):
    ...
    with R.start(
        ...
        #     file.write(report)
        # with open(f'./{logdir}/backtest_report_tau'+str(config["model_config"]["tau_hat_init"])+'.txt', 'w') as file:
        #     file.write(report)
        with open(f'./{logdir}/backtest_report_tau' + str(ind) + '.txt', 'w') as file:
            file.write(report)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_file", type=str, default="configs/f158_ts_baseline/config_master_ts_ori.yaml", help="config file") #
    ...
    old_logdir = config["logdir"]
    # for tau_hat_init in np.arange(-4, 5, 1): #np.arange(-4.9, 5, 0.4):
    #     config["model_config"]["tau_hat_init"] = tau_hat_init
    market_benchmark_pairs = [
        ("csi300", "SH000300"),
        ("csi800", "SH000906"),
    ]
    for market, benchmark in market_benchmark_pairs:
        config["market"] = market
        config["benchmark"] = benchmark
        config["data_handler_config"]["instruments"] = market
        config["port_analysis_config"]["backtest"]["benchmark"] = benchmark
        # Update logdir based on market
        base_logdir = old_logdir + f"_{market}"
        config["logdir"] = base_logdir
        config["task"]["model"]["kwargs"]["logdir"] = base_logdir
        for seq_len in [10, 40, 60, 90]:
            config["seq_len"] = seq_len
            config['model_config']['seq_len'] = seq_len
            config['task']['dataset']['kwargs']['seq_len'] = seq_len
            for lr in [0.001, 0.0001, 0.00001]:
                config["task"]["model"]["kwargs"]["lr"] = lr
                for bs in [1024, 512, 256]:
                    config["task"]["dataset"]["kwargs"]["batch_size"] = bs
                    for layers in [1, 2, 3]:
                        config["model_config"]["e_layers"] = layers
                        for d_model in [64, 128, 256]:
                            config["model_config"]["d_model"] = d_model
                            # for phi_type in ['phi_ones',
                            ...
                            print(
                                "index: {}, market: {}, benchmark: {}, lr: {}, bs: {}, layers: {}, d_model: {}".format(
                                    ind, market, benchmark, lr, bs, layers, d_model))
                            ind = ind + 1
                            config["model_config"]["ind"] = ind
                            main(args.seed, config, args.config_file, ind)
